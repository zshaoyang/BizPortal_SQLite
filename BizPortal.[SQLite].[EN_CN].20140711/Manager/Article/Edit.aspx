<%@ Page Language="C#" AutoEventWireup="false" Inherits="BizPortal.AdminUI.EditArticlePage" ValidateRequest="false" %>
        ...
        <div id="main">
            <div class="inner">
                <div class="main_title">
                    <strong><%=LangRes.Text("EditArticleData") %></strong>
                ...
                <div class="main_content">
                    <form id="EditorForm" action="<%=API_URL("AdminArticleHandler") %>?action=101" method="post" enctype="multipart/form-data">
                    <table class="editor_table" cellpadding="0" cellspacing="0">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td class="field" style="width:35%;"><input type="text" id="txtArticleTitle" name="Title" class="txt" maxlength="100" value="<%=TheArticle.Title %>" /></td>
                                ...
                                <td class="field" style="width:35%;">
                                    <select id="selectArticleEnableComment" name="EnableComment" class="select">
                                        <option value="False" <%=Helper.SELECTED("EnableComment", false, TheArticle.EnableComment) %>><%=LangRes.Text("No") %></option>
                                        <option value="True" <%=Helper.SELECTED("EnableComment", true, TheArticle.EnableComment) %>><%=LangRes.Text("Yes") %></option>
                                    </select>
                                </td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td class="field" colspan="3">
                                    ...
                                    <% foreach(ArticleCategory category in RootArticleCategoryList){ %>
                                        <option value="<%=category.CategoryId %>" <%=Helper.SELECTED("CategoryId", category.CategoryId, TheArticle.CategoryId) %>><%=category.Name %>(<%=category.UrlName %>)</option>
                                    <% } %>
                                    </select>
                                    ...
                                    <% 
                                    if(TheArticle.CategoryId > 0){    
                                        foreach(ArticleCategory level2Category in TheArticle.Category.ChildCategoryList){
                                    %>
                                        <option value="<%=level2Category.CategoryId %>" <%=Helper.SELECTED("Level2CategoryId", level2Category.CategoryId, TheArticle.Level2CategoryId) %>><%=level2Category.Name %>(<%=level2Category.UrlName %>)</option>
                                    <% 
                                        }
                                    }    
                                    %>
                            ...
                            <tr>
                                ...
                                <td class="field" colspan="3">
                                    <textarea id="txtArticleContent" name="Content" rows="20" cols="60" class="txt"><%=DEnCodeHelper.HtmlEncode(TheArticle.Content) %></textarea>
                                </td>
                            </tr>
                        ...
                        <tfoot>
                            ...
                            <tr>
                                <td colspan="4" style="text-align:center;">
                                    <input type="hidden" id="ArticleId" name="ArticleId" value="<%=TheArticle.ArticleId %>" />
                                    <input type="submit" value="<%=LangRes.Text("EditArticleData") %>" class="submit" />
                                </td>
                            </tr>
                        </tfoot>
                    </table>
    ...
    <script type="text/javascript">
        $(function(){
            ...
            $("#AttachFile").uploadify({
                ...
                'formData'          : { "TargetId": <%=TheArticle.ArticleId%>, "BelongType": "<%=AttachBelongTypes.Article %>", "__AdminAuth" : ADMIN_AUTH },
                'queueSizeLimit'    : 5,
                'fileSizeLimit'     : '2MB',
